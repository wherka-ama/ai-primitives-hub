...
describe('primitive-index / default-paths', () => {
  ...
  it('uses AI_PRIMITIVES_HUB_CACHE when set (highest precedence)', () => {
    const env: DefaultPathEnv = { AI_PRIMITIVES_HUB_CACHE: '/custom/cache' };
  ...
  it('falls back to ~/.cache/ai-primitives-hub when no env set', () => {
    ...
    expect(defaultCacheDir(env)).toBe(path.join(homeDir, '.cache', 'ai-primitives-hub'));
  });

  it('defaultIndexFile composes off defaultCacheDir', () => {
    const env: DefaultPathEnv = { AI_PRIMITIVES_HUB_CACHE: '/c' };
