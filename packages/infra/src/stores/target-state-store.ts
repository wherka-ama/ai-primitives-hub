 ...
 * - Desired-vs-actual state comparison (the `status` command)
 *
 * State is persisted at:
 * - Per-project: <project>/.ai-primitives-hub/target-state.json
 * - User-level: <XDG_CONFIG_HOME>/ai-primitives-hub/target-state.json (optional)
 *
 * (paths rebranded per migration plan §8 decision 3 — CLI-only,
 * unreleased artifact; callers resolve the actual path.)
 * @module stores/target-state-store
 */
