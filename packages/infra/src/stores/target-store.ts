 ...
 * project config file (`ai-primitives-hub.yml` or `ai-primitives-hub.yaml`).
 *
 ...
 * `ai-primitives-hub config list` to inspect and hand-edit until the
 ...
 *
 * Config filenames rebranded per migration plan §8 decision 3
 * (CLI-only, unreleased artifact — `prompt-registry.yml` ->
 * `ai-primitives-hub.yml`).
 * @module stores/target-store
 */
 ...
 * Falls back to `cwd/ai-primitives-hub.yml` when no config exists.
 * @param opts cwd / fs.
