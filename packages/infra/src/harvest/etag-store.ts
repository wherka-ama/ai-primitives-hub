 ...
 *
 * Uses `node:fs/promises` directly rather than `core`'s `FileSystem`
 * port: needs an atomic temp+rename write and a "missing/corrupt file
 * is not an error" open, both awkward to express through the port's
 * `readFile`/`writeFile`/`exists` trio. Ported unchanged from the
 * reference branch's `infra/src/github/etag-store.ts`.
 * @module harvest/etag-store
 */
