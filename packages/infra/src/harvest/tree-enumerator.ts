 ...
 *
 * Ported from the reference branch's `infra/src/harvest/tree-enumerator.ts`,
 * adapted to depend on `core`'s `GitHubApi` port (used by the whole
 * `infra` layer, adapters included) instead of a harvest-only
 * `GitHubClient` class — the reference's `GitHubClient` and our
 * `GitHubApiClient` expose the same `getJson`/`getJsonWithEtag` surface,
 * so no behavioural change.
 * @module harvest/tree-enumerator
 */
...
export function isPrimitiveCandidatePath(p: string): boolean {
  ...
  if (lower.endsWith('.vscode/mcp.json')) {
    return true;
  }
  if (lower.endsWith('collection.yml') || lower.endsWith('/collection.yml')
    || lower.endsWith('collection.yaml') || lower.endsWith('/collection.yaml')) {
    return true;
  }
  if (lower.endsWith('deployment-manifest.yml') || lower.endsWith('/deployment-manifest.yml')
    || lower.endsWith('deployment-manifest.yaml') || lower.endsWith('/deployment-manifest.yaml')) {
    return true;
  }
  return false;
}
