 ...
 * sequential appends, which the (per-call, no-handle) `FileSystem` port
 * doesn't model. Ported unchanged from the reference branch's
 * `infra/src/harvest/progress-log.ts`.
 * @module harvest/progress-log
 */
...
export class HarvestProgressLog {
  ...
  private apply(ev: ProgressEvent): void {
    ...
    if (ev.kind === 'start' && prev?.status === 'done' && prev?.commitSha === ev.commitSha) {
      return;
    }
