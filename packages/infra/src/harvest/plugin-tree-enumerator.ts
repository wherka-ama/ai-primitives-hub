 ...
 * `infra/src/harvest/plugin-tree-enumerator.ts`, adapted to depend on
 * `core`'s `GitHubApi` port instead of a harvest-only `GitHubClient`
 * class (see `tree-enumerator.ts`'s module doc for rationale).
 * @module harvest/plugin-tree-enumerator
 */
...
export interface EnumeratePluginRepoOptions {
  ...
  etagStore?: EtagStore;
  /**
   * Inject an existing BlobCache so manifest reads hit the same blob
   * cache the caller uses elsewhere. When omitted, manifests are fetched
   * directly without caching (tests typically omit this).
   */
  cache?: BlobCache;
...
async function fetchManifestResults(
  ...
  return Promise.all(
    manifestPaths.map(async (manifestPath) => {
      ...
      try {
        const bytes = await fetchManifestBytes(client, cache, owner, repo, manifestBlob.sha);
...
async function fetchTree(client: GitHubApi, owner: string, repo: string, commitSha: string): Promise<TreeResponse> {
