 ...
 * `FileSystem` port: needs raw `Buffer` reads/writes (not the port's
 * string-only `readFile`/`writeFile`) plus a temp+rename atomic write.
 * Ported unchanged from the reference branch's
 * `infra/src/github/blob-cache.ts`.
 * @module harvest/blob-cache
 */
