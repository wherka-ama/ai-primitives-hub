 ...
 * `infra/src/harvest/bundle-providers/local-folder.ts`; `BundleManifest`/
 * `BundleProvider`/`BundleRef` now come from `@ai-primitives-hub/core`.
 * Uses `node:fs` directly rather than `core`'s `FileSystem` port: the
 * `BundleProvider` interface's `listBundles`/`readManifest` need
 * synchronous directory/file existence checks inline with the async
 * generator's control flow, which the (fully async) port would only
 * complicate without buying anything — this provider is a CLI/harvest-only
 * leaf, not shared adapter logic.
 * @module harvest/bundle-providers/local-folder
 */
