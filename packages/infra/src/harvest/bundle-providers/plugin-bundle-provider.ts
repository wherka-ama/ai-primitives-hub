 ...
 * `infra/src/harvest/bundle-providers/plugin-bundle-provider.ts`,
 * adapted to depend on `core`'s `GitHubApi` port instead of a
 * harvest-only `GitHubClient` class (see `tree-enumerator.ts`'s module
 * doc for rationale) and `core`'s `HubSourceSpec`/`PluginManifest`/
 * `BundleManifest`/`BundleProvider`/`BundleRef`.
 * @module harvest/bundle-providers/plugin-bundle-provider
 */
