 ...
 * save path needs a synchronous write-to-temp + rename for atomicity,
 * which the (async, higher-level) `FileSystem` port doesn't model.
 * Ported unchanged from the reference branch's `infra/src/harvest/integrity.ts`.
 * @module harvest/integrity
 */
