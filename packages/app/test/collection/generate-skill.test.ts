/**
 * Ported unchanged (behaviorally) from the reference branch's
 * `app/test/collection/app-collection-generate-skill.test.ts`, save for
 * inlining the tiny `createTempDir` helper (reference imports it from a
 * shared `../helpers/install-test-helpers` grab-bag; this is the only
 * consumer on this branch so far, so it isn't worth standing up a new
 * shared helper module for one caller).
 */
