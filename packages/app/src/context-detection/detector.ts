 ...
 * `app/src/context-detection/detector.ts`, including its direct
 * `node:fs/promises` usage (not routed through `core`'s `FileSystem`
 * port) and its three still-stubbed activity signals
 * (`getRecentFiles`/`getGitBranch`/`getLastCommitMessage`, each
 * documented in the reference as "for now" placeholders) — ported
 * as-is per this migration's behavior-parity discipline, not
 * opportunistically completed or re-plumbed through ports here.
 * @module context-detection/detector
 */
