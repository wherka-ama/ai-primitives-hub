 ...
 * `app/src/discovery/recommendation-engine.ts`. Per that branch's own
 * code comment, `generateFallbackRecommendations` is itself still a
 * stub there (computes search queries but discards them, always
 * returns `[]`, "will be integrated with primitive index search in a
 * future iteration") — ported as-is, not completed here; the CLI's
 * own `discover` command (Phase 5) performs the real, working
 * non-AI search path directly against `infra`'s `PrimitiveIndex`
 * rather than through this stub.
 * @module discovery/recommendation-engine
 */
