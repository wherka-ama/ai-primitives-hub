 ...
 *
 * Covers the two access patterns `src/adapters/github-adapter.ts` and
 * `src/adapters/skills-adapter.ts` need today: JSON GETs (repository
 * contents, tree, releases) and text GETs (raw file content). Deliberately
 * lean — conditional/ETag GETs and rate-limit telemetry are not ported yet
 * since nothing on `main` uses them; extend this port when Phase 3 actually
 * needs them (e.g. porting the Git Trees API perf optimization from
 * `c1fbb24`), not speculatively now.
