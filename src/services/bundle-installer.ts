...
export class BundleInstaller {
  ...
  public async installFromBuffer(
    ...
    const writer: TargetWriter = {
      write: async (_target: Target, files: ExtractedFiles): Promise<TargetWriteResult> => {
        ...
        if (installSkillsToCopilotDir) {
          ...
          // Find the skill name - look for the first directory under skills/ in the bundle.
          const skillsPrefix = 'skills/';
          const skillPaths = [...files.keys()].filter((p) => p.startsWith(skillsPrefix));
          if (skillPaths.length === 0) {
            throw new Error('Skills directory not found in bundle');
          }
          const skillName = skillPaths[0].slice(skillsPrefix.length).split('/')[0];
          ...
          const skillEntryPrefix = `skills/${skillName}/`;
    ...
    try {
      ...
      // Install MCP servers if defined (skip for skills bundles)
      if (installSkillsToCopilotDir) {
        ...
        // Sync to appropriate scope directory (skills for repository scope must run through this)
