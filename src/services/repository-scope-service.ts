 ...
 * (migration plan §7.5 item 2) instead of duplicating it.
 */
...
export class RepositoryScopeService implements IScopeService {
   ...
   * `FileTreeTargetWriter.writeManifestItems()` (migration plan §7.5
   * item 2), called once per manifest item so this service's own
  ...
  private async installSkillAndTrack(
    ...
    const sourceDir = path.join(bundlePath, 'skills', sourceSkillName);
    ...
    const files = await this.readDirectoryIntoMap(sourceDir, `skills/${sourceSkillName}`);
