 ...
 * (app/, infra/, cli/, registry-config/) depend on domain — never the reverse."
 *
...
const FEATURE_LAYER_DIRECTORIES = new Set([
  ...
  'cli',
  'public',
  // Legacy directories (removed in Phase 2)
  'primitive-index',
  'hub',
  'core',
  'registry',
...
const rule = {
  ...
  create(context) {
    ...
    if (!matchesPattern(filename, '/lib/src/domain/')) {
      return {};
    }
    return {
